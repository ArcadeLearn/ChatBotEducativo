...
export function CourseDetailCard({ data }: CourseDetailCardProps) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-slate-900/60">
      {/* Hero — texto claro sobre imagen/gradiente oscuro en ambos modos */}
      <div className="course-detail-hero relative">
        {data.thumbnail ? (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          <h4 className="edu-hero-text text-base font-bold">{data.title}</h4>
          {data.instructor && (
            <p className="edu-hero-subtext text-xs">{data.instructor}</p>
          )}
        </div>
        <div className="absolute right-4 top-4 flex h-16 w-16 flex-col items-center justify-center rounded-full border-4 border-sky-400/80 bg-white/95 dark:bg-slate-950/90">
          <span className="text-lg font-bold text-sky-600 dark:text-sky-300">
            {data.progressPercentage}%
          </span>
          <span className="text-[9px] text-slate-500 dark:text-slate-400">avance</span>
        </div>
      </div>

      {/* Stats row */}
      <div className="grid grid-cols-3 gap-2 border-b border-slate-200 p-4 dark:border-white/5">
      ...
      {stats.pendingLessonTitle && (
        <div className="mx-4 mt-3 rounded-lg border border-amber-500/40 bg-amber-50 px-3 py-2 dark:border-amber-500/30 dark:bg-amber-500/10">
          <p className="text-xs font-medium text-amber-800 dark:text-amber-300">
            Siguiente actividad pendiente
          </p>
          <p className="mt-0.5 text-xs text-amber-900/90 dark:text-amber-100/80">
            {stats.pendingLessonTitle}
          </p>
        </div>
      )}
          ...
          return (
            <div
              ...
              className={`rounded-xl border p-3 ${
                isComplete
                  ? "border-teal-500/40 bg-teal-50 dark:border-teal-500/30 dark:bg-teal-500/5"
                  : isPending
                    ? "border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-950/40"
                    : "border-sky-500/40 bg-sky-50 dark:border-sky-500/30 dark:bg-sky-500/5"
              }`}
            >
...
function StatBox({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-100 py-2 text-center dark:border-white/5 dark:bg-slate-950/50">
      <p
        className={`text-lg font-bold ${
          accent ? "text-teal-600 dark:text-teal-400" : "text-slate-900 dark:text-white"
        }`}
      >
        {value}
      </p>
